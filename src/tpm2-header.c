 ...
 */
TPM2_ST
tpm2_header_get_tag (uint8_t* hdr)
{
    return be16toh (*(TPM2_ST*)hdr);
}
/*
 * Set the 'tag' field in a response buffer.
 */
void
tpm2_header_set_tag (uint8_t* hdr, TPM2_ST tag)
{
    *(TPM2_ST*)hdr = htobe16 (tag);
}
/**
 ...
 */
UINT32
tpm2_header_get_size (uint8_t* hdr)
{
    return be32toh (*(UINT32*)(hdr + sizeof (TPM2_ST)));
}
/*
 * Set the 'size' field in a response buffer.
 */
void
tpm2_header_set_size (uint8_t* hdr, UINT32 size)
{
    *(UINT32*)(hdr + sizeof (TPM2_ST)) = htobe32 (size);
}
/**
 ...
 */
TPM2_CC
tpm2_header_get_command_code (uint8_t* hdr)
{
    return be32toh (*(TPM2_CC*)(hdr + sizeof (TPM2_ST) + sizeof (UINT32)));
}
/**
 ...
 */
TSS2_RC
tpm2_header_get_response_code (uint8_t* hdr)
{
    return be32toh (*(TSS2_RC*)(hdr + sizeof (TPM2_ST) + sizeof (UINT32)));
}
/*
 * Set the responseCode field in a TPM response buffer.
 */
void
tpm2_header_set_response_code (uint8_t* hdr, TSS2_RC rc)
{
    *(TSS2_RC*)(hdr + sizeof (TPM2_ST) + sizeof (UINT32)) = htobe32 (rc);
}
/*
